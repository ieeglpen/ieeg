...
%% (6) Hago la correccion por multiples comparaciones.
%(6.1) Primero me quedo con todos los valores p sacando los NaNs y guardo
%su lugar original
...
%% (7) Calculo cual es el valor p que me deja en el percentil elegido por el usuario
%calculo el thres pvals en funcion de todos los valores p obtenidos
aux_pvals_lugares=find(Resultado.P<=0.05);
aux_pvals=Resultado.P(aux_pvals_lugares);
%percentil elige cual es el valor que deja por arriba a X% de la muestra,
%ejemplo, el percentil 20 elije un valor que deja por arriba al 20% de la
%muestra de valores p
thres_pval= prctile(aux_pvals,percentil);
clear aux_pvals aux_pvals_lugares
...
for i=lugares_pvals
    ...
    Tabla{cont,4}=adj_p(find(i==pvals_lugar_multi));
    Tabla{cont,5}=nanmean(unos_out(:,i));
    Tabla{cont,6}=nanmean(dos_out(:,i));       
end

%% (9) Reconstruyo una matriz con las etiquetas vs teniendo en cuenta TODAS las areas ingresadas
...
%% (10) Ahora me tomo el trabajo de buscar todas las areas que encontre significativas
%y me guardo en que area de la matriz esta
Tabla{1,7}='Lugares_atlas';

for i=1:length(Tabla(:,1))-1
  
    Tabla{i+1,7}=find(strcmp(Tabla{i+1,1}, atlas_label));

end

%% (11) Ahora creo las tres tablas para los controles separandolos de los pacientes
%(11.1) Primero saco todas las que son mayores en los controles
Cont_high{1,1}='AreasVs';
Cont_high{1,2}='pvals';
Cont_high{1,3}='T_test';
Cont_high{1,4}='P_FDR_corr';
Cont_high{1,5}='Rho_Cont';
Cont_high{1,6}='Rho_Pac';
Cont_high{1,7}='Lugares_atlas';

cont=0;
for i=1:length(Tabla(:,1))-1
    if Tabla{i+1,5} > Tabla{i+1,6}
        cont=cont+1;
        ...
        Cont_high{cont+1,7}=Tabla{i+1,7};
    else
    end
end
%(11.2) Ahora me quedo solo con las comparaciones positivas
Cont_highPos{1,1}='AreasVs';
Cont_highPos{1,2}='pvals';
Cont_highPos{1,3}='T_test';
Cont_highPos{1,4}='P_FDR_corr';
Cont_highPos{1,5}='Rho_Cont';
Cont_highPos{1,6}='Rho_Pac';
Cont_highPos{1,7}='Lugares_atlas';
cont=0;
for i=1:length(Cont_high(:,1))-1
    
    if Cont_high{i+1,5}<0 && Cont_high{i+1,6}<0
    else
        cont=cont+1;
        ...
        Cont_highPos{cont+1,7}=Cont_high{i+1,7};
    end
end
%(11.3) Ahora me quedo solo con las negativas
Cont_highNeg{1,1}='AreasVs';
Cont_highNeg{1,2}='pvals';
Cont_highNeg{1,3}='T_test';
Cont_highNeg{1,4}='P_FDR_corr';
Cont_highNeg{1,5}='Rho_Cont';
Cont_highNeg{1,6}='Rho_Pac';
Cont_highNeg{1,7}='Lugares_atlas';
cont=0;
for i=1:length(Cont_high(:,1))-1
    ...
    if Cont_high{i+1,5}<0 && Cont_high{i+1,6}<0
        ...
        Cont_highNeg{cont+1,7}=Cont_high{i+1,7};
    else
    end
end
...
%% (12) Creo la matriz para los sujetos poniendo TODO el atlas para CONTROLES
%empiezo por highPOS
matriz(1:length(rois_lista),1:length(rois_lista))=0;
matriz([Controles.GroupOne_highPos{2:end,7}])=1;
matriz_aux=transpose(matriz);
Controles.matGUHighPos=matriz_aux+matriz;
clear matriz matriz_aux
%empiezo con los highNeg
matriz(1:length(rois_lista),1:length(rois_lista))=0;
matriz([Controles.GroupOne_highNeg{2:end,7}])=1;
matriz_aux=transpose(matriz);
Controles.matGULessNEG=matriz_aux+matriz;
clear matriz matriz_aux

%% (13) Ahora creo las tres tablas para los PACIENTES separandolos de los pacientes
%(13.1) Primero saco todas las que son mayores en los controles
Pac_high{1,1}='AreasVs';
Pac_high{1,2}='pvals';
Pac_high{1,3}='T_test';
Pac_high{1,4}='P_FDR_corr';
Pac_high{1,5}='Rho_Cont';
Pac_high{1,6}='Rho_Pac';
Pac_high{1,7}='Lugares_atlas';
...
for i=1:length(Tabla(:,1))-1
    if Tabla{i+1,5} < Tabla{i+1,6}
        ...
        Pac_high{cont+1,7}=Tabla{i+1,7};
    else
    end
end
%(13.2) Ahora me quedo solo con las comparaciones positivas
Pac_highPos{1,1}='AreasVs';
Pac_highPos{1,2}='pvals';
Pac_highPos{1,3}='T_test';
Pac_highPos{1,4}='P_FDR_corr';
Pac_highPos{1,5}='Rho_Cont';
Pac_highPos{1,6}='Rho_Pac';
Pac_highPos{1,7}='Lugares_atlas';
cont=0;
for i=1:length(Pac_high(:,1))-1
    
    if Pac_high{i+1,5}<0 && Pac_high{i+1,6}<0
    else
        cont=cont+1;
        ...
        Pac_highPos{cont+1,7}=Pac_high{i+1,7};
    end
end
%(13.3) Ahora me quedo solo con las negativas
Pac_highNeg{1,1}='AreasVs';
Pac_highNeg{1,2}='pvals';
Pac_highNeg{1,3}='T_test';
Pac_highNeg{1,4}='P_FDR_corr';
Pac_highNeg{1,5}='Rho_Cont';
Pac_highNeg{1,6}='Rho_Pac';
Pac_highNeg{1,7}='Lugares_atlas';
cont=0;
for i=1:length(Pac_high(:,1))-1
    ...
    if Pac_high{i+1,5}<0 && Pac_high{i+1,6}<0
        ...
        Pac_highNeg{cont+1,7}=Pac_high{i+1,7};
    else
    end
end
...
%% (14) La matriz para los sujetos poniendo TODO el atlas para los PACIENTES
%empiezo por highPOS
matriz(1:length(rois_lista),1:length(rois_lista))=0;
matriz([Pacientes.GrupoDos_highPos{2:end,7}])=1;
matriz_aux=transpose(matriz);
Pacientes.matGUHighPos=matriz_aux+matriz;
clear matriz matriz_aux
%empiezo con los highNeg
matriz(1:length(rois_lista),1:length(rois_lista))=0;
matriz([Pacientes.GrupoDos_highNeg{2:end,7}])=1;
matriz_aux=transpose(matriz);
