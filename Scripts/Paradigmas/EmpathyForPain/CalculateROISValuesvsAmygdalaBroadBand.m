function [labels,trialsResults] = CalculateROISValuesvsAmygdalaBroadBand(EmpathyForPainStruct,ROIINT,ROIACC,AmygdalaDiff,timeWindow,frequencyIndex,baseTimeWindow,pvalue)
        ...
        [t df pvals] = statcond({rvalerpsINTACC,rvalerpsINTACCBase}, 'mode', 'bootstrap','paired','off','tail','both','naccu',1000);   %calcula permutaciones
            ...
            if pvals > pvalue || (pvals < pvalue & (meanValueACC > meanValueINT || meanINTACCBase >= meanINTACC))
                ...
                if pvals < pvalue & meanAmygdalaINTACC > meanINTACC
                    ...
                    broadband2INT = valerpsINT;
                    broadband2ACC = valerpsACC;
                    pbroadband2INT = permute(valerpsINT,[2 3 1]);
