...
if size(timeWindow) == [1 2]    
    ...
    for m = 1 : size(timeWindowPositions,1)
        
        timeWindowColumns = timeWindowPositions(m,:);
        values2Plot = GetValues2Plot(resultMean,timeWindowColumns,connectivityThreshold);
        
        timeValues(m) = (EjeX(timeWindowColumns(1,1)) + EjeX(timeWindowColumns(1,2)))/2;
        ...
        completeConnectionMatrix(:,m) = nrOfConnections;
