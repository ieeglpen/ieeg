function PlotHeadConnectivityForConditions(signalFileName,electrodeInfoFile,threshold,initTime,timeWindow,titleName)
...
if size(timeWindow) == [1 2]    
    ...
    for i = 1 : size(conditions,1)
        ...
        % mean by frequency
        result = squeeze(conditions(i,:,:,:));
        resultMean = mean(result,3);

        values = GetValues2Plot(resultMean,timeWindowColumns,threshold);
        
        values2Plot(i).conditionName = InfoCond.conditionName;
        values2Plot(i).color = InfoCond.color;
        values2Plot(i).binarizedArray = values;
