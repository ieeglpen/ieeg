...
for i = 1 : size(uniqueDistances,1)
    ...
    %[t df pvals] = statcond(data, 'mode', 'perm', 'naccu', 1000,'paired','off','tail','both');
...
% f1=figure;
% plot(uniqueDistances, cond1UniqueValues,'r')
% hold on
% plot(uniqueDistances, cond2UniqueValues, 'b')
% hold on
% 
% if ~isempty(i_ind )    
%     i_ind = i_ind(i_ind >= distanceThresholdPlot);
%     plot(uniqueDistances(i_ind), 0.07, '*', 'Color', 'k', 'MarkerSize', 5)
% end
