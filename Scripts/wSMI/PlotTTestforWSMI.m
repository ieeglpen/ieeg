...
%RESHAPE MATRICES FOR STATISTICAL ANALYSIS FOR AMYGDALA
C_1 = [];
C_2 = [];

amygdalaMatrix = zeros(channelNr,channelNr);
amygdalaMaskedMatrix = zeros(channelNr,channelNr);

amygdalaIndex = [29];
%amygdalaIndex = [37 38];

%amygdalaIndex = [32 33 34];
%amygdalaIndex = [34];

%amygdalaIndex = [14]; %abv
%amygdalaIndex = [11]; %abv
%amygdalaIndex = [17]; %abv

temp = M.C1(amygdalaIndex,:,:);
C_1 = [C_1,reshape(temp,length(amygdalaIndex)*size(temp,2),size(temp,3))];
%C_1 = temp;

temp = M.C2(amygdalaIndex,:,:);
C_2 = [C_2,reshape(temp,length(amygdalaIndex)*size(temp,2),size(temp,3))];
%C_2 = temp;
...
%[stats df pvals surrog] = statcond({C_1,C_2},'paired','off','method','perm','naccu',200);
%[stats df pvals surrog] = statcond({C_1,C_2},'method','param');
...
P = reshape(p,length(amygdalaIndex),channelNr);

sustractionMatrix = mean(M.C1,3) - mean(M.C2,3);

amygdalaMatrix(amygdalaIndex,:) = sustractionMatrix(amygdalaIndex,:);

intRawValues = length(find(amygdalaMatrix>0));
accRawValues = length(find(amygdalaMatrix<0));

% str = ['En tau ' int2str(tau) ' con p value = ' num2str(p_value) ' hay intRaw: ' int2str(intRawValues) ' vs accRaw: ' int2str(accRawValues)];
% display(str)
...
dosave = 1;
PlotFormattedMap(amygdalaMatrix,labels,caxisvalues,colorMap,titlename,titlename,dosave);

%BINARIZE MATRIX TO PLOT HISTOGRAM 
amygdalaMatrix(amygdalaMatrix>0) = 1;
amygdalaMatrix(amygdalaMatrix<0) = -1;

posVec = ones(length(find(amygdalaMatrix>0)),1);
negVec = -1*ones(length(find(amygdalaMatrix<0)),1);
...
%PLOT AMYGDALA STATISCAL P VALUES
display(['Cantidad de valores significativos ' int2str(length(find(P < p_value)))])
find(P < p_value)

amygdalaMaskedMatrix2plot = sustractionMatrix(amygdalaIndex,:);
amygdalaMaskedMatrix2plot(P > p_value) = 0;
amygdalaMaskedMatrix(amygdalaIndex,:) = amygdalaMaskedMatrix2plot;

caxisvalues = [-0.01 0.01];
colorMap = 'jet';
titlename = [newFileName 'amygdala-mask-' condition1 '-' condition2];
dosave = 1;
PlotFormattedMap(amygdalaMaskedMatrix,labels,caxisvalues,colorMap,titlename,titlename,dosave);


%BINARIZE MATRIX TO PLOT HISTOGRAM OF SIGNIFICANT VALUES
amygdalaMaskedMatrix(amygdalaMaskedMatrix>0) = 1;
amygdalaMaskedMatrix(amygdalaMaskedMatrix<0) = -1;

posVec = ones(length(find(amygdalaMaskedMatrix>0)),1);
negVec = -1*ones(length(find(amygdalaMaskedMatrix<0)),1);
Vec = cat(1,posVec,negVec);

str = ['En tau ' int2str(tau) ' con p value = ' num2str(p_value) ' hay int: ' int2str(length(posVec)) ' vs acc: ' int2str(length(negVec))];
display(str)
