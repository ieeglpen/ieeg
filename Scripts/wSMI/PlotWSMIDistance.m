...
%nr_sig_dif=pvals(pvals<p_value);
...
% f1=figure;
% plot(data_cond1_ordered(distanceThresholdPlot:end,2), data_cond1_ordered(distanceThresholdPlot:end,1))
% hold on
% plot(data_cond2_ordered(distanceThresholdPlot:end,2), data_cond2_ordered(distanceThresholdPlot:end,1), 'r')
% hold on
% if ~isempty(i_ind )    
%     i_ind = i_ind(i_ind >= distanceThresholdPlot);
%     plot(data_cond1_ordered(i_ind,2), 0.07, '*', 'Color', 'k', 'MarkerSize', 5)
% end
...
%     valuesForDistance = data_cond1_ordered(find(data_cond1_ordered(:,2)== distance,1));
%     meanValueForDistance = mean(valuesForDistance);
%     cond1UniqueValues(i) = meanValueForDistance;
%     
%     valuesForDistance = data_cond2_ordered(find(data_cond2_ordered(:,2)== distance,1));
%     meanValueForDistance = mean(valuesForDistance);
%     cond2UniqueValues(i) = meanValueForDistance;
% end

% f1=figure;
% plot(uniqueDistances, cond1UniqueValues)
% hold on
% plot(uniqueDistances, cond2UniqueValues, 'r')
% hold on
% % if ~isempty(i_ind )    
% %     i_ind = i_ind(i_ind >= distanceThresholdPlot);
% %     plot(uniqueDistances(i_ind,2), 0.07, '*', 'Color', 'k', 'MarkerSize', 5)
% % end
% title(['Tau= ' num2str(tau) ' - ' Task])
% 
% f1=figure;
% plot(uniqueDistances, smooth(cond1UniqueValues,80))
% hold on
% plot(uniqueDistances, smooth(cond2UniqueValues,80), 'r')
% hold on
% % if ~isempty(i_ind )    
% %     i_ind = i_ind(i_ind >= distanceThresholdPlot);
% %     plot(uniqueDistances(i_ind,2), 0.07, '*', 'Color', 'k', 'MarkerSize', 5)
...
hold on
%plot(data_cond2_ordered(distanceThresholdPlot:end,2), smooth(data_cond2_ordered(distanceThresholdPlot:end,1), 80), 'r')
