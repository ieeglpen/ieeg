...
function [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mypop_newtimef(EEG, typeproc, num, tlimits, cycles,freqRange,alpha,fdrCorrect,scale,baseline, varargin );
varargout{1} = '';
lastcom = num;
...
if alpha == 0
    if fdrCorrect == 1    
        if ~isempty(baseline)
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',baseline, 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1, 'basenorm', basenormvalue,'mcorrect','fdr');
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue,'mcorrect','fdr');
        else
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1, 'basenorm', basenormvalue,'mcorrect','fdr');
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue,'mcorrect','fdr');
        end
    else
        if ~isempty(baseline)
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue);
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',baseline, 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'basenorm', basenormvalue);
        else
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue);
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'basenorm', basenormvalue);
        end
    end
else
    if fdrCorrect == 1
        if ~isempty(baseline)
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue,'mcorrect','fdr');                                                                                                                                                         
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',baseline, 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1, 'basenorm', basenormvalue,'mcorrect','fdr'); 
        else
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue,'mcorrect','fdr');                                                                                                                                                         
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1, 'basenorm', basenormvalue,'mcorrect','fdr');                                                                                                                                                         
        end
    else
        if ~isempty(baseline)
            display('baseline is NOT empty')
            baseline
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue);
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',baseline, 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1, 'basenorm', basenormvalue);
        else
            display('baseline IS empty')
            %[P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1,'scale',scale, 'basenorm', basenormvalue);
            [P,R,mbase,timesout,freqs,Pboot,Rboot,alltfX,PA,ERP,maskersp, maskitc, g,Pboottrials] = mynewtimef( tmpsig(:, :), length(pointrange), [tlimits(1) tlimits(2)], EEG.srate, cycles , 'baseline',[0], 'freqs', [freqRange], 'alpha',alpha, 'plotitc' , 'off', 'plotphase', 'off', 'padratio', 1, 'basenorm', basenormvalue);
        end
    end    
end
